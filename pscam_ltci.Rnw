...
logistic regression to examine each outcome. Due to small sample size, we were only able to use
occurrence of delirium as a covariate in each model, so these results are not adjusted for any
confounders.
...
fit.delay.mod <- function(outcome){
  ## Fit model: outcome ~ delirium in ICU
  mod.fit <- do.call(lrm,
                     list(formula = as.formula(paste(outcome, '~ ever.del')),
                          data = peds.ltci.data))
...
delay.results <- do.call(rbind, lapply(delay.outcomes, FUN = fit.delay.mod))
